...
export const buildDocument = (cleanTemplate, route, rendered) => {
...
const prerender = async () => {
  ...
  try {
    ...
    for (const route of prerenderRoutes) {
      ...
      await writeFile(outputPath, buildDocument(cleanTemplate, route, render(route)), 'utf8');
    }

    await writeFile(join(distDir, '404.html'), buildDocument(cleanTemplate, '/404', render('/404')), 'utf8');
